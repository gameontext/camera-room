 ...
 */
@ServerEndpoint("/cameraRoom")
@WebListener
public class CameraRoom implements ServletContextListener {
    ...
    private static final String name = "CameraRoom";
    private static final String fullName = "A room with a device to capture images";
    ...
    public CameraRoom() {
        ...
        endPointUrl = getJNDIEntry("thisSvcUrl");
    ...
    private void processCommand(Session session, String json) throws IOException {
        ...
        String test = "/test";
        if(lowerContent.equals(test)) {
            String description = null;
            if((imageSvcUrl == null) || imageSvcUrl.isEmpty()) {
                description = "hhmmm, no image service is available to test with.";
            } else {
                CameraRoomImage pic = CameraRoomImage.getInstance("testimage", "testimage");
                if(pic.getImage().length == 0) {
                    description = "hhmm, couldn't find image";
                } else {
                    ...
                    ByteArrayBody body = new ByteArrayBody(pic.getImage(), ContentType.APPLICATION_OCTET_STREAM, "testimage");
            ...
            EventBuilder.playerEvent(Collections.singletonList(session), userId, description, null);
            return;
        }
